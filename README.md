...
[![crates.io](https://img.shields.io/crates/v/drm3-rpc-pool)](https://crates.io/crates/drm3-rpc-pool)
[![docs.rs](https://img.shields.io/docsrs/drm3-rpc-pool)](https://docs.rs/drm3-rpc-pool)
[![npm](https://img.shields.io/npm/v/%40drm3labs-oss%2Frpc-pool)](https://www.npmjs.com/package/@drm3labs-oss/rpc-pool)
[![downloads](https://img.shields.io/crates/d/drm3-rpc-pool)](https://crates.io/crates/drm3-rpc-pool)
...
<!-- BENCHMARK:START -->
_No CI benchmark has run yet. Trigger the **Benchmark** workflow (or wait for the weekly run) to replace this block with live, timestamped numbers and chart. The table below is an illustrative sample of the result shape (not a real measurement)._

![Throughput: without vs with the pool](./assets/benchmark.svg)

| Mode | Success rate | Throughput (req/s) | p50 latency | p95 latency |
|------|-------------:|-------------------:|------------:|------------:|
| Without pool (single endpoint) | _sample_ ~60% | _sample_ | _sample_ | _sample_ |
| With pool (failover) | _sample_ ~100% | _sample_ | _sample_ | _sample_ |
